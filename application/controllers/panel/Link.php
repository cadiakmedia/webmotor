...
class Link extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'page_title'        => 'Panel',
            'url'               => 'panel/link/get_data_link',
            'url1'               => 'panel/link/edit_link',
            'url3'               => 'panel/link/input_link',
            'data_user'         => $Auth,
            ...
            'js'                => ['back/jsphp/link.php',],
        ];
        $this->load->helper('url');
       
        $this->admin($data, 'back/konten/panel/link.php');
    }
    function get_data_link()
    {
        $csrf_name = $this->security->get_csrf_token_name();
        $csrf_hash = $this->security->get_csrf_hash();  
        $list = $this->M_link->get_datatables();
        // var_dump($list);
        // die;
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) {
            ...
            <a href="" id="edit"  data-target="#edit_link" data-toggle="modal" data-id='.$field->id.' class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_link->count_all(),
            "recordsFiltered" => $this->M_link->count_filtered(),
            "data" => $data,
